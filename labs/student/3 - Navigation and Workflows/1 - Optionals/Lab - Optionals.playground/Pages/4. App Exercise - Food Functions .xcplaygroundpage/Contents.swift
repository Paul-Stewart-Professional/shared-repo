...
func mealFood(meal: String) -> Meal? {
    if let mealItem = meals[meal] {
        return mealItem
    } else {
        return nil
    }
...
if let breakfastMeal = mealFood(meal: "Breakfast") {
    print("Food for Breakfast: \(breakfastMeal.food)")
    print("Calories for Breakfast: \(breakfastMeal.calories)")
} else {
    print("Meal has not yet been logged")
}
/*:
 ...
 */
func retrieveMealLog() {
    if let mealLog = UserDefaults.standard.dictionary(forKey: "mealLog") {
        // mealLog contains the retrieved meal log
        print(mealLog)
    } else {
        // No meal log has been saved
